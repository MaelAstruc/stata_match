...
function write_pkg(
    ...
    fput(fh_out, "v 3")
    fput(fh_out, "")
    fput(fh_out, "d {bf:PATMATCH}: Pattern matching in Stata.")
    fput(fh_out, "d")
    fput(fh_out, "d Distribution-Date: " + date_fmt)
    fput(fh_out, "")
    fput(fh_out, "f patmatch.ado")
    fput(fh_out, "f patmatch.sthlp")
    fput(fh_out, "f patmatch.pdf")
...
function write_pkg_details(real scalar fh, string scalar pkg_version) {
    ...
    fput(fh, `""')
    fput(fh, `"Version      : {bf:patmatch} version "' + pkg_version)
    fput(fh, `"Source       : {browse "https://github.com/MaelAstruc/patmatch":GitHub}"')
    fput(fh, `""')
...
function write_feedback(real scalar fh) {
    ...
    fput(fh, `""')
    fput(fh, `"{p}Please submit bugs, errors, feature requests on {browse "https://github.com/MaelAstruc/patmatch/issues":GitHub} by opening a new issue, or by sending me an email.{p_end}"')
}
...
function write_citation(
    ...
    fput(fh, "")
    fput(fh, "{p}Astruc--Le Souder, M. (" + strofreal(year(date), "%04.0f") + "). Stata package 'patmatch' version " + pkg_version + " https://github.com/MaelAstruc/patmatch.{p_end}")
    fput(fh, "")
    fput(fh, "@software{patmatch,")
    fput(fh, "   author = {Astruc--Le Souder Mael},")
    fput(fh, "   title = {Stata package ``patmatch''},")
    fput(fh, "   url = {https://github.com/MaelAstruc/patmatch},")
